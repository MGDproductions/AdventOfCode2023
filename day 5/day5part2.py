# WARNING, THIS CODE IS EXTREMELY, AND I MEAN REALLY EXTREMELY INEFFICIENT!!!!! IT TAKES AN UNGODFUL AMOUNT OF TIME TO RUN THIS!!!!

f = open("input.txt", "r")
line = f.readline()
lowest = None

originalseeds = line.replace("\n", "").split(": ")[1].split(" ")
seeds = []

skip = False
for i in range(len(originalseeds)):
    if skip == False:
        base = int(originalseeds[i])
        seedrange = int(originalseeds[i + 1])
        skip = True
        
        seeds.extend(range(base, base + seedrange))
    else:
        skip = False
